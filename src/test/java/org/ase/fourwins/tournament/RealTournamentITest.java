...
public class RealTournamentITest {
	...
	@Property
	void jqwikCheckTestRandom(
			@ForAll @IntRange(min = 0, max = MAX_PLAYERS) int playerCount,
			@ForAll @IntRange(min = 0, max = MAX_SEASONS) int seasons) {
		List<PlayerMock> players = createPlayers(playerCount,
				RandomMockPlayer::new);
		playSeasons(seasons, players).forEach(this::verifyGameState);
		verifyPlayers(players, seasons);
		verifySumOfPoints(players.size(), seasons);
	}
	...
	@Property
	void jqwikCheckTestKeepTrack(
			@ForAll @IntRange(min = 0, max = MAX_PLAYERS) int playerCount,
			@ForAll @IntRange(min = 0, max = MAX_SEASONS) int seasons) {
		List<PlayerMock> players = createPlayers(playerCount,
				ColumnTrackingMockPlayer::new);
		playSeasons(seasons, players).forEach(this::verifyGameState);
		verifyPlayers(players, seasons);
		verifySumOfPoints(players.size(), seasons);
	}

	List<PlayerMock> createPlayers(int players,
			Function<String, PlayerMock> function) {
		return IntStream.range(0, players).mapToObj(String::valueOf)
				.map("P"::concat).map(function).collect(toList());
	}

	Stream<GameState> playSeasons(int seasons, List<PlayerMock> players) {
		Tournament tournament = registerPlayers(new DefaultTournament(),
				players);
	...
	Stream<GameState> playSeasons(int seasons, Tournament tournament) {
		return IntStream.range(0, seasons)
				.mapToObj(i -> tournament.playSeason()).flatMap(identity());
	}

	protected Tournament registerPlayers(Tournament tournament,
			Collection<PlayerMock> players) {
	...
	void verifyPlayers(Collection<PlayerMock> players, int numberOfSeasons) {
		players.forEach(p -> {
			int expectedJoinedMatches = (players.size() - 1) * numberOfSeasons
					* 2;
	...
	void verifyGameState(GameState score) {
		if (score.getScore() == WIN) {
			assertThat(String.valueOf(score),
					score.getWinningCombinations().size(), is(not(0)));
		}
		if (score.getScore() != WIN) {
			assertThat(String.valueOf(score),
					score.getWinningCombinations().size(), is(0));
		}
		if (score.getScore() == LOSE) {
			assertThat(String.valueOf(score), score.getReason().isEmpty(),
					is(false));
		}
	}
