 ...
 * {@code java.lang.System}. It provides support for
 * <ul>
 ...
 * <li>Suppress output to {@link #muteSystemErr(Statement) System.err} and/or
 * {@link #muteSystemOut(Statement) System.out}</li>
 * <li>Tap output that is written to {@link #tapSystemErr(Statement) System.err}
 * and/or {@link #tapSystemOut(Statement) System.out}. Also tapped output with
 * normalized line separator for {@link #tapSystemErrNormalized(Statement)
 * System.err} and/or {@link #tapSystemOutNormalized(Statement)
 * System.out}.</li>
...
public class SystemLambda {
	...
	/**
	 * Executes the statement and fails (throws an {@code AssertionError}) if the
	 * statement tries to write to {@code System.err}.
	 * <p>
	 ...
	 * </pre>
	 * 
	 * The test fails with the failure "Tried to write 's' to System.err although
	 * this is not allowed."
	 *
	 * @param statement an arbitrary piece of code.
	 * @throws Exception any exception thrown by the statement or an
	 *                   {@code AssertionError} if the statement tries to write to
	 *                   {@code System.err}.
	...
	/**
	 * Executes the statement and fails (throws an {@code AssertionError}) if the
	 * statement tries to write to {@code System.out}.
	 * <p>
	 * The following test fails
	 * 
	 * <pre>
	 ...
	 * visible. It may even slowdown the test. {@code muteSystemErr} can be used to
	 ...
	 * visible. It may even slowdown the test. {@code muteSystemOut} can be used to
	 * suppress this output.
	 * 
	 * <pre>
	 * &#064;Test
	...
	/**
	 * {@code tapSystemErrNormalized} returns a String with the text that is written
	 * to {@code System.err} by the provided piece of code. New line characters are
	...
	public static String tapSystemErrNormalized(Statement statement) throws Exception {
		return tapSystemErr(statement).replace(lineSeparator(), "\n");
	}

	/**
	 * {@code tapSystemOutNormalized} returns a String with the text that is written
	 * to {@code System.out} by the provided piece of code. New line characters are
	 * replaced with a single {@code \n}.
	 * 
	 * <pre>
	 * &#064;Test
	...
	public static String tapSystemOutNormalized(Statement statement) throws Exception {
		return tapSystemOut(statement).replace(lineSeparator(), "\n");
	}

	/**
	 * Executes the statement and restores the system properties after the statement
	 * has been executed. This allows you to set or clear system properties within
	 * the statement without affecting other tests.
	 * 
	 * <pre>
	 * &#064;Test
	 * public void execute_code_that_manipulates_system_properties() throws Exception {
	 * 	System.clearProperty("some property");
	 * 	System.setProperty("another property", "value before test");
	 *
	 * 	restoreSystemProperties(() -&gt; {
	 * 		System.setProperty("some property", "some value");
	 * 		assertEquals("some value", System.getProperty("some property"));
	 *
	 * 		System.clearProperty("another property");
	 * 		assertNull(System.getProperty("another property"));
	 * 	});
	 *
	 * 	// values are restored after test
	 * 	assertNull(System.getProperty("some property"));
	 * 	assertEquals("value before test", System.getProperty("another property"));
	 * }
	 * </pre>
	 * 
	 * @param statement an arbitrary piece of code.
	...
	/**
	 * Executes a statement with the specified environment variables. All changes to
	 * environment variables are reverted after the statement has been executed.
	 * 
	 * <pre>
	 * &#064;Test
	 * public void execute_code_with_environment_variables() throws Exception {
	 * 	withEnvironmentVariable("first", "first value").and("second", "second value").and("third", null)
	 * 			.execute(() -&gt; {
	 ...
	 * <p>
	 * <b>Warning:</b> This method uses reflection for modifying internals of the
	 * environment variables map. It fails if your {@code SecurityManager} forbids
	 * such modifications.
	 * 
	 * @param name  the name of the environment variable.
	 * @param value the value of the environment variable.
	 * @return an {@link WithEnvironmentVariables} instance that can be used to set
	 *         more variables and run a statement with the specified environment
	 *         variables.
	 * @since 1.0.0
	 ...
	 * 
	 * @param securityManager the security manager that is used while the statement
	 *                        is executed.
	 * @param statement       an arbitrary piece of code.
	 * @throws Exception any exception thrown by the statement.
	 * @since 1.0.0
	...
	/**
	 * Executes the statement and lets {@code System.in} provide the specified text
	 * during the execution. In addition several Exceptions can be specified that
	 * are thrown when {@code System.in#read} is called.
	 *
	 * <pre>
	 *   public void MyTest {
	 *
	 ...
	 * <p>
	 * You can also simulate a {@code System.in} that throws an {@code IOException}
	 * or {@code RuntimeException}. Use
	 *
	 * <pre>
	 *   public void MyTest {
	 *
	 ...
	 * </pre>
	 * <p>
	 * If you provide text as parameters of {@code withTextFromSystemIn(...)} in
	 * addition then the exception is thrown after the text has been read from
	 * {@code System.in}.
	 * 
	 * @param lines the lines that are available from {@code System.in}.
	 * @return an {@link SystemInStub} instance that is used to execute a statement
	 *         with its {@link SystemInStub#execute(Statement) execute} method. In
	 *         addition it can be used to specify an exception that is thrown after
	 *         the text is read.
	 * @since 1.0.0
	...
	public static SystemInStub withTextFromSystemIn(String... lines) {
		String text = stream(lines).map(line -> line + getProperty("line.separator")).collect(joining());
		return new SystemInStub(text);
	}

	private static void executeWithSystemErrReplacement(OutputStream replacementForErr, Statement statement)
			throws Exception {
	...
	private static void executeWithSystemOutReplacement(OutputStream replacementForOut, Statement statement)
			throws Exception {
	...
	private static PrintStream wrap(OutputStream outputStream) throws UnsupportedEncodingException {
		return new PrintStream(outputStream, AUTO_FLUSH, DEFAULT_ENCODING);
	}

	private static class DisallowWriteStream extends OutputStream {
		@Override
		public void write(int b) {
			throw new AssertionError("Tried to write '" + (char) b + "' although this is not allowed.");
		}
	}
	...
	/**
	 * A collection of values for environment variables. New values can be added by
	 * {@link #and(String, String)}. The {@code EnvironmentVariables} object is then
	 * used to execute an arbitrary piece of code with these environment variables
	 * being present.
	 */
	public static final class WithEnvironmentVariables {
		...
		/**
		 * Creates a new {@code WithEnvironmentVariables} object that additionally
		 ...
		 * @throws IllegalArgumentException when a value for the environment variable
		 *                                  {@code name} is already specified.
		...
		private void validateNotSet(String name, String value) {
			if (variables.containsKey(name)) {
				...
				throw new IllegalArgumentException("The environment variable '" + name + "' cannot be set to "
						+ format(value) + " because it was already set to " + format(currentValue) + ".");
			}
		}
		...
		/**
		 * Executes a statement with environment variable values according to what was
		 * set before. All changes to environment variables are reverted after the
		...
		private void overrideVariables(Map<String, String> existingVariables) {
			if (existingVariables != null) // theCaseInsensitiveEnvironment may be null
				variables.forEach((name, value) -> set(existingVariables, name, value));
		}
		...
		private static Map<String, String> getEditableMapOfVariables() {
			...
			try {
				...
				throw new RuntimeException("System Rules cannot access the field" + " 'm' of the map System.getenv().",
						e);
		...
		/*
		 * The names of environment variables are case-insensitive in Windows. Therefore
		 * it stores the variables in a TreeMap named theCaseInsensitiveEnvironment.
		 */
		private static Map<String, String> getTheCaseInsensitiveEnvironment() {
			try {
				...
				throw new RuntimeException("System Rules expects the existence of"
						+ " the class java.lang.ProcessEnvironment but it does not" + " exist.", e);
			} catch (IllegalAccessException e) {
				throw new RuntimeException("System Rules cannot access the static"
						+ " field 'theCaseInsensitiveEnvironment' of the class" + " java.lang.ProcessEnvironment.", e);
			} catch (NoSuchFieldException e) {
	...
	public static class SystemInStub {
		...
		public SystemInStub andExceptionThrownOnInputEnd(IOException exception) {
			if (runtimeException != null)
				throw new IllegalStateException("You cannot call" + " andExceptionThrownOnInputEnd(IOException) because"
						+ " andExceptionThrownOnInputEnd(RuntimeException) has" + " already been called.");
		...
		public SystemInStub andExceptionThrownOnInputEnd(RuntimeException exception) {
			if (ioException != null)
				throw new IllegalStateException(
						"You cannot call" + " andExceptionThrownOnInputEnd(RuntimeException) because"
								+ " andExceptionThrownOnInputEnd(IOException) has already" + " been called.");
		...
		private static class ReplacementInputStream extends InputStream {
			...
			private boolean isCompleteLineWritten(byte[] buffer, int indexLastByteWritten) {
				byte[] separator = getProperty("line.separator").getBytes(defaultCharset());
